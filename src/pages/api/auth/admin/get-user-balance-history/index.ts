...
import { connectAndExecute, getUserBalanceHistory } from '@/libs/NineDragonsAccount';
  ...
  try {
    ...
    if (type === 'credit') {
      ...
      await connectAndExecute(async (pool) => {
        balances = await getUserBalanceHistory(pool);
      });
    ...
    if (type === 'debit') {
      const balances = await DatabaseCis.getAllItemsShopHistory();
      const response: BaseResponse = {
        status: 200,
        success: true,
        message: 'Get data success',
