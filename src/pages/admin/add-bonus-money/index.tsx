...
import { addBonusMoneyAction } from '@/stores/admin';
...
export default function AddBonusMoney() {
  const dispatch = useAppDispatch();

  const handleSubmit = async (values: FormData, formikHelpers: FormikHelpers<FormData>) => {
    const result: any = await dispatch(addBonusMoneyAction({ ...values })).unwrap();

    if (result?.status === 200) {
      toast.success('Nạp tiền vào tk khuyến mãi thành công!');
    ...
    <AdminLayout>
      ...
      >
        <div className="flex w-full flex-row items-center  justify-center bg-[#3b5998]">
          <span className="text-5xl font-bold text-yellow">Nạp tiền khuyến mại cho User</span>
        </div>
