...
import { addItemBonusAction } from '@/stores/admin';
...
function AddItem() {
  ...
  const handleSubmit = async (values: FormData, formikHelpers: FormikHelpers<FormData>) => {
    ...
    const result: any = await dispatch(addItemBonusAction(payload)).unwrap();
    ...
    <AdminLayout>
      ...
      >
        <div className="flex w-full flex-row items-center  justify-center bg-[#3b5998]">
          <span className="text-5xl font-bold text-yellow">Thêm Vật phẩm long ngân các mới</span>
        </div>
        <div className="flex w-full items-center justify-center ">
          <span className="flex items-center divide-y py-4 text-center text-2xl text-black">
            Vui lòng nhập thông tin cho Vật phẩm long ngân các mới
          </span>
        </div>
