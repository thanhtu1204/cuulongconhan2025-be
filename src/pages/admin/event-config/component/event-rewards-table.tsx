...
import { deleteEventRewardsAction, getAllEventRewardsAction } from '@/stores/admin';

const EventRewardsTable = () => {
  ...
  const currentItems: any[] = listEventRewards.slice(indexOfFirstItem, indexOfLastItem);
  ...
  async function onClickDelete(param: number | string) {
    const result: any = await dispatch(deleteEventRewardsAction(param as string)).unwrap();
    if (result?.status === 200) {
      ...
      <div className="relative z-0 mx-12 mb-6 flex w-full min-w-0 flex-col  break-words shadow-lg ">
        <div className="z-0 block w-full overflow-x-auto">
              ...
              {flatMap(currentItems, (item) => (
                <tr key={item.event_id}>
                  <th className=" items-center whitespace-nowrap border-x-0 border-t-0 p-4 px-6 text-left align-middle text-xs">
                    <span className="text-center text-xs text-white">{item?.event_id}</span>
                  </th>
                  <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                    <MyLazyLoadedImage
                      src={item?.background_image || itemDefault.src}
                      className="h-8 w-8"
                      alt="Logo"
                    />
                  </td>
                  ...
                  <td className="max-w-[200px] whitespace-pre-wrap break-words border-x-0 border-t-0 p-4 px-6 align-middle text-xs text-white">
                    {moment.unix(Number(item.start_time) / 1000).format('DD/MM/YYYY HH:mm:ss')}
                  </td>
                  <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                    {moment.unix(Number(item.end_time) / 1000).format('DD/MM/YYYY HH:mm:ss')}
                  </td>
          ...
          <Pagination
            ...
            totalPages={listEventRewards.length}
            onPageChange={paginate}
          />
        </div>
      </div>
      <ModalConfirm
        title="Bạn có chắc chắn event?"
        isOpen={openModal}
...
export default memo(EventRewardsTable);
