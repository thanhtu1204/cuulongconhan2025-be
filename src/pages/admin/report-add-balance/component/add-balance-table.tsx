...
const AddBalanceTable = () => {
  ...
  const getData = useCallback(async () => {
    await dispatch(getUserBalanceHistoryAction('credit'));
  }, [dispatch]);
            ...
            {flatMap(currentItems, (item) => (
              <tr key={item.history_id}>
                <th className=" items-center whitespace-nowrap border-x-0 border-t-0 p-4 px-6 text-left align-middle text-xs">
                  <span className="text-center text-xs text-white">{item.history_id}</span>
                </th>
                <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                  <span className="text-center text-xs text-white">{item?.user_name}</span>
                </td>
                <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                  {item?.email}
                </td>
                ...
                <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                  {numberWithDot(item?.old_balance || 0)}
                </td>
                <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                  {numberWithDot(item?.new_balance || 0)}
                </td>
                <td
                  className={`whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  ${
                    item?.transaction_type === 'credit' ? 'text-green-600' : 'text-red-500'
                  }`}
                >
                  {item?.transaction_type === 'credit' ? 'Cộng tiền' : 'Trừ tiền'}
                </td>
                <td className="whitespace-nowrap border-x-0 border-t-0 p-4 px-6 align-middle text-xs  text-white">
                  {moment
                    .unix(Number(item?.change_time_timestamp) / 1000)
                    .utc()
...
export default memo(AddBalanceTable);
